...
All temporary authentication tokens expire after 10 minutes and are single-use only. Once logged in, the user sessions will be valid for 30 days.
...
Modification of the `TOOL_CALL_APPROVAL_TIMEOUT_MS` environment variable allows you to override how long AnythingLLM waits for a user to approve or deny an agent tool-call before the request times out.

By default, the timeout is **120 000 ms (2 minutes)**. If a user does not respond within this window, the tool call is automatically rejected.

### Enable

Set the `TOOL_CALL_APPROVAL_TIMEOUT_MS` environment variable to a timeout value in **milliseconds**. The value must be a number greater than `0`.

```bash
TOOL_CALL_APPROVAL_TIMEOUT_MS="300000" # 5 minutes
```

### Disable

Fully remove or comment out the `TOOL_CALL_APPROVAL_TIMEOUT_MS` environment variable to return to the default behavior (2 minutes).
