...
### Introducing Model Router — The World's First Hybrid AI Experience

**This is a first.** No other platform offers what AnythingLLM is shipping today.

The Model Router is the first-ever user-defined intelligent routing system that seamlessly blends local and cloud AI into a single, unified experience — entirely under your control. Until now, you had to choose: run everything locally, or send everything to the cloud. That tradeoff is over.
...
- **You're in complete control.** Create [calculated rules](/model-router/setup#calculated-rules) that trigger on keywords, token counts, time of day, or image attachments — instant and free. Or use [LLM-classified rules](/model-router/setup#llm-classified-rules) that understand intent in plain English.
...
This isn't just a feature — it's a fundamental shift in how AI assistants work. For the first time, you get the privacy of local models, the power of cloud models, and the intelligence to know when to use each. And it's 100% open source.
