...
AnythingLLM can also build memories for you automatically. A background job reviews your recent chats every so often and turns useful facts into workspace memories so you don't have to write them by hand.

**How it works**

- The job looks at your unprocessed chats in each workspace and sends them to that workspace's LLM.
- The LLM returns an updated list of up to 20 workspace memories, combining anything new with what you already had and dropping anything that's been contradicted by newer messages.
- That list replaces your workspace memories for that workspace. Only workspace memories are written this way. Global memories are only ever added or edited by you.
- Chats that have been reviewed are marked so they aren't looked at again.

**When it runs**

- On a schedule (default: every **15 minutes**).
- Only when you've been idle. If anyone on the instance has chatted in the last **20 minutes**, the job skips that round so it doesn't process a conversation that's still going.
...
This happens for regular chats, streamed chats, and chats made through the API.
...
| `MEMORY_IDLE_THRESHOLD_MS`   | `1200000` | How long (in milliseconds) the instance has to be idle before a run can start. Default is 20 minutes. |
