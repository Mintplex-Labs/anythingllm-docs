...
AnythingLLM limits how many tools an agent can chain in one response, but that
limit is a safety guard. A skill should still return enough information for the
model to determine that the requested work is complete.
