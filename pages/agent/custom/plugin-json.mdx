...
```js
{
  // see #schema-reference for more information
...
A [JSON Schema](https://json-schema.org/) for `plugin.json` is published in the AnythingLLM repository. Pointing your editor at it gives you autocomplete, inline descriptions, and validation while you write the manifest - most editors, including VS Code and the JetBrains IDEs, pick it up automatically.

The simplest way to use it is to add a `$schema` key as the first property of your `plugin.json`:
...
If you would rather not commit the extra key, you can map the schema to your `plugin.json` files in your editor settings instead. In VS Code that looks like this:
  ...
  "json.schemas": [
    {
      ...
      "url": "https://raw.githubusercontent.com/Mintplex-Labs/anything-llm/refs/heads/master/server/utils/agents/imported-manifest.schema.json"
    }
  ]
}
```
...
<Callout type="info" emoji="️💡">
  `$schema` is ignored by AnythingLLM when the skill is loaded, so it is safe to
  leave in a published skill. It exists purely for your editor.
</Callout>
