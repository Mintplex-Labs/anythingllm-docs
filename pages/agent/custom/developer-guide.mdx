...
<Callout type="info" emoji="️💡">
  ...
  same `name` property as the associated `plugin.json` file.
</Callout>
### Docker
...
Your custom agent skill should be wrapped in a folder with the same `name` property that is defined in the `plugin.json` file.

```js
// example plugin.json
{
  "name": "my-custom-agent-skill",
  "hubId": "my-custom-agent-skill" // THIS MUST BE THE SAME AS THE NAME PROPERTY & MUST EXIST.
  // see here for more information on the plugin.json file https://docs.anythingllm.com/agent/custom/plugin-json-reference
}
```

Folder structure for associated agent skill:
NOTE: The folder name must match the `name` property in the `plugin.json` file.
```text
