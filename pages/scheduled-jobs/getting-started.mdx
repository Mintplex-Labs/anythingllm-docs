...
- **Only pick what the job needs.** A focused tool list makes the agent faster and more reliable.
- **Turn on web search** if the job needs current information from the internet.
- **Turn on document search** if the job should reference your embedded documents.
- **Add your MCP servers or agent flows** only if the job actually uses them.
