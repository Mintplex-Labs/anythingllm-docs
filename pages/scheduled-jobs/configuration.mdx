...
- **Start with the defaults.** `SCHEDULED_JOB_MAX_CONCURRENT=1` and a 5-minute timeout are reasonable starting points for most setups.
- **Watch the first few runs.** After creating a new job, open its run history and inspect the first couple of runs to confirm the agent is doing what you want and not eating unexpected cost.
- **Use the `Enabled` toggle as a kill switch.** If a job starts misbehaving, disable it from the jobs list instead of deleting it. You keep the run history and can re-enable it once you've fixed the prompt.
- **Stagger schedules.** If you have many jobs, avoid scheduling them all at the top of the hour — stagger their minute offsets so they don't all compete for the same concurrency slot.
