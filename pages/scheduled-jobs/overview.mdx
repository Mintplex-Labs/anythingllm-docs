...
Pick a prompt, pick a schedule, pick which tools the agent is allowed to use, and let it run. Every time a job runs, AnythingLLM records what the agent thought, which tools it called, any files it produced, and its final response — all browsable from the Scheduled Jobs page.
...
Anything you can do by talking to an agent in a chat, you can schedule — with the added benefit that results are archived, searchable, and can be continued as a normal workspace thread whenever you want to follow up.
